...
def train(opts:Any=None):
    ...
    if tacc >= 0.95:
        name = f'{opts.models_dir if opts and opts.models_dir else "."}{os.sep}pycf-{tacc:.04f}-{MTYPE}-{F}x{K}x{len(DILATIONS)}-{DROPOUT}-w{WINDOW_BEFORE}x{WINDOW_AFTER}-{int(time.time())}.keras'
        print()
